...
export function ExpedienteCard({ expediente, user, onTransfer, onViewHistory }) {
  ...
  // ✅ usar areaActual, NO expediente.area
  const canTransfer = expediente.areaActual === user.area;

  // ✅ áreas disponibles usando areaActual
  const availableAreas = Object.entries(AREAS).filter(
    ([key]) => key !== expediente.areaActual
  );
  ...
  const loadUsersForArea = async (area) => {
    try {
      ...
      if (filteredUsers.length > 0) {
        setSelectedUser(filteredUsers[0]._id);
      }
    } catch (err) {
      ...
      <div className="p-6">
        ...
        <div className="flex items-center space-x-4">
          <div className="flex items-center space-x-1">
            ...
            <span>Creado el: {expediente.fechaCreacion}</span>
          </div>
          <div className="flex items-center space-x-1">
            <Clock className="w-4 h-4" />
            <span>Hora: {expediente.horaCreacion}</span>
          </div>
        </div>

        <div className="flex items-center justify-between text-sm text-gray-600 mb-4">
          <span className="text-xs">
            Por: {expediente.createdBy?.name || expediente.createdBy?.username || 'Desconocido'}
          </span>
        </div>
